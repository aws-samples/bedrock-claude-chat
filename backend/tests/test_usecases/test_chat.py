    ...
    def test_language_titles(self):
      for lang_code, text in self.example_texts.items():
          with self.subTest(lang_code=lang_code):
              ...
              title, detected_language = propose_conversation_title(self.user_id, output.conversation_id, lang_code)
              self.assertEqual(detected_language, lang_code, f"Expected language {lang_code}, but got {detected_language}")
              detected_title_lang = detect(title)
              self.assertEqual(detected_title_lang, lang_code, f"Title language mismatch: expected {lang_code}, detected {detected_title_lang}")
