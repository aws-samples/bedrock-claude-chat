...
- 35 分ほど経過後、〜./bin を実行する際にオプションパラメータの--disable-self-register を使用してセルフサインアップを無効にしてデプロイしてください。
