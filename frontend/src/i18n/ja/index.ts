...
const translation = {
  translation: {
    ...
    agent: {
      ...
      hint: 'エージェントは、ユーザーの質問に答えたり、より複雑なタスクを処理したりするために、どのツールを使用するかを自動的に判断します。実行できるタスクの複雑さのため、応答時間が長くなる場合があります。たとえ1つのツールをアクティブにしても、エージェントの機能が有効になります。逆に、ツールが選択されていない場合、エージェントの機能は利用されません。エージェントの機能がアクティブになると、クエリに応答するために知識ベースを使用するかどうかを自動的に決定します。',
      tools: {
        get_weather: {
          name: '現在の天気',
